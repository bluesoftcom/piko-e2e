...
@Component
@Slf4j
public class PikoLocationsClient {

    private final HttpClient httpClient = HttpClient.newHttpClient();
    private final String url;
    private final ObjectMapper objectMapper;

    public PikoLocationsClient(@Value("${piko-locations.url}") String url, ObjectMapper objectMapper) {
        this.url = url;
    ...
    public JsonResponse listLocations(CognitoUser user) {
        ...
        checkSuccessfulStatusCode(response, "list-locations");
    ...
    @SneakyThrows
    public JsonResponse createLocation(CognitoUser user, JsonNode body) {
        final HttpRequest request = HttpRequest.newBuilder()
        ...
        checkSuccessfulStatusCode(response, "create-location");
        return toJsonResponse(
                response,
                objectMapper.readValue(response.body(), JsonNode.class)
        );
    ...
    public JsonResponse changeLocationStatus(CognitoUser user, String locationId, JsonNode body) {
        ...
        checkSuccessfulStatusCode(response, "change-location-status");
        return toJsonResponse(
                response,
                objectMapper.readValue(response.body(), JsonNode.class)
        );
